...
export default function DocumentsPage() {
  ...
  const refreshUI = () => {
    if (searchMode && query.trim()) {
      handleSearch(); // relance la recherche immédiatement
    } else {
      loadDocuments();
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    refreshUI(); // 🔥 mise à jour instantanée
  };

  const handleEdit = (doc: Document) => {
    setEditId(doc.id as number);
    setTitle(doc.title);
  ...
  const handleDelete = async (id: number) => {
    ...
    refreshUI(); // 🔥 mise à jour instantanée après suppression
  };
